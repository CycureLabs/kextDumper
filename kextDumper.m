...
//  kextdumper.m
//
//  Created by Cycure Labs on 13/09/18.
//
...
int IOService_vtable_limit = 166;
...
int isUserClient; //Determine whether the currently analyzed IO class is a UserClient class.
int isInKEXTnow; //Distinguish between the current analysis kernel or the kernel extension

//machoH、File related function
    ...
    for(j=0;j<count;j++){
        if(count > 0){
            ...
            if(strstr(insn[j].mnemonic,"adr")){
                ...
                if(acount>0){
                    //The adrp instruction reads an address into the register
                    //eg. adrp x19,#0xffffff801c137000
                    //Just need to judge from the presence or absence of immediate, because adrp does not have multiple registers
                    uint64_t *xx = NULL;
                    ...
                    if (acount) {
                        if(acount>2){
                            printf("0x%llx adrp's immediate instruction register is greater than 2\n",insn[j].address);
                            return -1;
                        }
                    ...
                    if(s_reg==0){
                        printf("0x%llx adrp's immediate instruction did not get the first register\n",insn[j].address);
                        return -1;
                    }
            ...
            if(strstr(insn[j].mnemonic,"ldr")){
                ...
                if(reg_acount==1){
                    //Filtering: ldr instruction has only one register
                    
                    int imm_acount = cs_op_count(handle, &insn[j], ARM64_OP_MEM);
                    //Filtering: The ldr directive contains a memory offset operation
                    if(imm_acount){
                        //ldr instruction reads the second immediate value plus the memory pointed to by the offset to the first register value.
                        //eg. ldr x16, [x16,#0x80]
                        ...
                        if(xx){
                            *xx = *xx + offset;
                            ...
                            printf("0x%llx, Unable to get offset register when ldr instruction is addressed\n",insn[j].address);
                            return -1;
                        }
                        ...
                        if(!mem){
                            printf("ldr Unable to find memory at the specified location\n");
                            return -1;
                        }
...
//Get str/ldr instruction memory offset
#pragma mark imp: Get the memory offset of the str/ldr instruction
int32_t getMEMOPoffset(csh handle,const cs_insn *insn){
    int32_t i,offset;
...
//Get the offset register of the str/ldr instruction
#pragma mark imp: Get the offset register of the str/ldr instruction
int getMEMOPregister(csh handle,const cs_insn *insn){
    uint32_t i,offset;
    int acount = cs_op_count(handle, insn, ARM64_OP_MEM);
    if (acount) {
...
//Get the immediate number of a single instruction
#pragma mark imp: Get the immediate number of a single instruction
uint64_t getSingleIMM(csh handle,const cs_insn *insn){
    int i;
    uint64_t imm;
...
#pragma mark imp: Get the first register
int getFirstReg(csh handle,const cs_insn *insn){
    int i,s_reg;
...
#pragma mark imp: Get the second register
int getSecondReg(csh handle,const cs_insn *insn){
    int i,s_reg;
    int acount = cs_op_count(handle, insn, ARM64_OP_REG);
    if (acount) {
...
//Get the corresponding variable according to the register name
#pragma mark imp: Get the corresponding variable according to the register name
uint64_t* getActualVarFromRegName(uint64_t address,int RegName){
    switch (RegName) {
        case ARM64_REG_X0:
            return &x0;
...
//Convert the compiled virtual memory address and return the actual content in memory
#pragma mark imp: Convert the compiled virtual memory address and return the actual content in memory
void* getMemFromAddrOfVM(void* bin,uint64_t CurFunc_FilebaseAddr,uint64_t CurFunc_VMbaseAddr,uint64_t cur_VMAddr){
...
//Analyze the contents of the IO class, such as the contents of _const sec
#pragma mark imp: Analyze the contents of the IO class, such as the contents of _const sec
int ParseConstFunc(char **cn,uint64_t class_self,uint64_t class_super,void *bin,uint64_t VMaddr,uint64_t fileoff){
    ...
    if(!strcmp(*cn,"")){
        ...
        uint64_t vtable_start = 0; //The starting position of the vtable for this class can be used to analyze important function rewrites, etc.
        uint64_t vtable_checkItSuperClassAddr = 0; //Check its parent class address
        
        //In the special case of IOUserClent, the function table of IOUserClient does not have its own address at the top. Its own address is far before several classes.....
        
        //update(3.29.2016): Now adapt to arm64
        
        //Note: The previous IOUserclient needs to be treated differently, but if the analysis rewrites the function OSObject:alloc, it does not need to be treated differently.
        if(class_super!=0){
            for(uint64_t cur_addr = VMaddr;cur_addr>=__const_start;){
                //Here is the extra check after the IOUserClient, try to find the match of the parent class address in the memory, in this case, usually the address found above should be 0x4 bytes
            ...
            if(strcmp(*cn,"OSObject")&&strcmp(*cn,"OSMetaClass")){
                //Filter out OSObject, and there is also a parent class for OSMetaClass
...
#pragma mark part:Filter the rewritten function
        
        //Let's determine the class name to get the function information of the base class and assign it to the global variable.
        ...
        if(!strcmp(*cn,"IOUserClient")){
            ...
            for(int i = 0;i<vtable_count;i++){
                ...
                if(!check_PointerAddrInVM((uint64_t)p1)){
                    printf("Failed to create IOUserClient function table:0x%llx\n",(uint64_t)p1);
                    return -1;
                }
                (*cur_func).func_vm = *p1;
                /*
...
#pragma mark edit:Judge class rewrite function
        //Judge class rewrite function
        if(frIOUserClient){
            ...
            for(int i = 0;i<vtable_count;i++){
                ...
                if(!check_PointerAddrInVM((uint64_t)own_func)){
                    printf("Failed to judge when rewriting the function table:0x%llx\n",(uint64_t)own_func);
                    return -1;
                }
            ...
            for(int i = 0;i<IOService_vtable_limit+1;i++){
                ...
                if(*own_func!=cur_func->func_vm){
                    ...
                    if(!strcmp(cur_func->func_name,"IOService_newUserClientWithOSDic")){
                        ...
                        //find_openType(*cn,bin,*own_func,getfileoffFromAddrOfVM(fileoff,VMaddr,*own_func));
                        needAdd_MK_NEW_USERCLIENT = 0;
                    ...
                    /*if(FuzzIOKit_phase1_export&&needAdd_MK_NEW_USERCLIENT){
                     ...
                     if(!old_mark){
                     printf("fuzz_new_class_list didn't actually added FuzzIOKit_phase1!\n");
                     return -1;
                     }
        ...
        /*if(frIOHIDEventService){
         ...
         printf("copyEvent is overridden!!!!! loc:0x%llx imp:0x%llx\n",vtable_start + offset,*own_copyEvent);
                ...
                for(int mi = 0;;){
                       ...
                       ((*check_func_0 > __text_start)&&(*check_func_0 < __text_end))&&(*check_scalar_i_0 < 0xffff||*check_scalar_i_0 == 0xffffffff)&&(*check_struct_i_0 < 0xffff || *check_struct_i_0 == 0xffffffff)&&(*check_scalar_o_0 < 0xffff || *check_scalar_o_0 == 0xffffffff) && (*check_scalar_i_0 < 0xffff || *check_scalar_i_0 == 0xffffffff))
                    {
                        ...
                        if(printMethodsInfo){
                            ...
                            //printf("%d scalar_i:0x%x  struct_i:0x%x  scalar_o:0x%x  struct_o:0x%x\n",mi,*check_scalar_i_0,*check_struct_i_0,*check_scalar_o_0,*check_struct_o_0); 
                        }
                        mi++;
                    }
                    else{
...
uint64_t AnalysisAllocOfClass(void *bin_kern, uint64_t vmaddr, uint64_t fileoff){
    ...
    if(count > 0){
        ...
        for(j=0;j<count;j++){
#pragma mark ALLOC_DEBUG:Output assembly
            printf("0x%"PRIX64":\t%s\t\t%s\n",insn[j].address,insn[j].mnemonic,insn[j].op_str);
                        ...
                        if(acount==1){
                            ...
                            if(xx){
                                *xx = imm;
                                ...
                                printf("orr's mov synonym operation does not get a register\n");
                                return -1;
                            }
                        }
                    }
...
#pragma mark ALLOC_DEBUG:STR OP
                 ...
                 *mem = *tar_reg; 
                 ...
                 printf("str unable to find specified location\n");
                 return -1;
                 }*/
            }
...
int FindKEXTsThenAnalysis(void *kr_bin){
    ...
    if(fileoff==0||filesize==0||vmoff==0){
        printf("FindKEXTsThenAnalysis Kernel binary __PRELINK_TEXT information error\n");
        return -1;
    }
...
#pragma mark KEXT_LIST:List all kernel extensions(Add filter information)
...
#pragma mark KEXT_LIST:Export a separate kext
            ...
            uint64_t target_vm = 0xffffff801bff18d8; 
...
char *KextGetBundleID(void *kextmacho, void *bin_kern){
    //bin_kern Starting address for kext's macho
...
int AnalysisModInitOfKernel(void *kr_bin){
    ...
    for(int ab=1;ab<modInitSize/8;ab++){
        ...
        if(count > 0){
            ...
            for(j=0;j<count;j++){
#pragma mark KER_DEBUG:Kext output assembly
                //printf("0x%"PRIX64":\t%s\t\t%s\n",insn[j].address,insn[j].mnemonic,insn[j].op_str);
...
int AnalysisModInitOfKEXT(void *bin, void *bin_kern){
    ...
    if(printModInitQt)
        printf("\ntotal %llu modInit in %s\n",modInitSize/8,KextGetBundleID(bin, bin_kern)); 
...
#pragma mark KEXT_DEBUG:STR OP
                    ...
                    if(!tar_reg){
                        printf("0x%llx str instruction does not get the first register\n",insn[j].address);
                        return -1;
                    }
